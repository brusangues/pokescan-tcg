...
| **Teste de integração E2E** (`script/teste_integracao.py`) — Playwright contra o site publicado ou build local; 10 checks (assets, índice, motor, scan, /card set+num, card_id, liga_only, busca). Revelou P1.35 + descartou P1.36. Skill `pokescan-scanner-integracao` | `ed63a23` + `0819260` |
| **Índice único de carta (migração)** — chave canônica `en_id` p/ EN e `{idE}-{num}` p/ liga_only em TODO o site (cards_basico + build_search_index set.id). Elimina 524 colisões de `{sigla}-{num}`; cards.json 37.680 com 0 colisões; /card resolve liga_only por set+num e card_id. TAG `pre-idx-unico` | `bac57da` |
| **Resultado do scan abaixo da imagem (P2.48)** — no mobile o resultado da foto renderizava na coluna/aba "Buscar" (e o scan de UMA carta caía no estado da busca por texto), então parecia que a identificação tinha falhado. Agora o resultado renderiza na própria aba Foto, logo abaixo da imagem, com título explícito ("N cartas encontradas na imagem"), aviso "Procurando cartas na imagem…" durante o processamento e rolagem automática até o resultado; a aba Buscar ficou só com a busca por texto. Junto: status do motor de "Descarregado" → **"Motor não carregado"**, e o número cru de debug de cada detecção (`3214.0224019132165px`) saiu da UI — foi para o tooltip do aviso "carta pequena" | `33cb516` |
...
| **Descoberta de edições da Liga (fronteira + ranking de variação)** — o `discover_set_ids()` só sondava ±5 em volta de uma lista base e a faixa 760–779, então a fronteira terminava em 784: **51 edições novas** (incluindo as comemorativas **30C**, **30C-C**, **30THP**, **30C-R**, **M6/M6a**, **C9.5C**, **PBL**, **PLAY**…) ficavam fora do catálogo, do índice e dos hits, mesmo já aparecendo no ranking de variação da própria Liga. Agora sonda a fronteira acima do maior idE e lê as edições novas direto do ranking. Catálogo **31.281 → 35.986** cartas; índice do scanner **37.679 → 41.792**; site **37.680 → 41.793**; os hits já trazem cartas de 30C/30C-C/M6 | `33291cc` + `ed36d3f` |
| **Orçamento diário/semanal da macro** — a diária passou a ser limitada por ambiente (`LIGA_FRONTEIRA=10`, `LIGA_GAP_SLICE=0`, `SNAP_MAX_SETS=450`, `EMB_LIMITE=2000`), com **janela rotativa** de sets (antes o teto pegaria sempre os primeiros N e as últimas edições nunca seriam refrescadas) e heartbeat com hora em cada passo; o **semanal** (`macro_semanal.sh`, job `macro-semanal-pokemon`, domingo 02:00) roda com orçamento cheio (fronteira 80, buracos 400, todos os sets, embeddings sem teto) e timeout de job de 6h. Motivo: com 793 sets o passo de snapshot (que crawleia todos os sets/dia) dobrou de 35 para ~70 min e a diária estourou o limite de 2h | `8a96926` |
...
| **Preço por condição/tipo dos anúncios + tipo, idioma e custo por unidade (P2.44)** — o enriquecedor agora guarda o preço dos anúncios **por condição × tipo** (`v3m.pc`, 430 cartas): `qualid` (M/NM/SP/MP/HP/D) × `extras` (Normal/Foil/especial), tudo em texto plano no HTML da carta. Cada unidade da coleção ganhou **tipo**, **idioma** (PT/EN/JP — a mesma carta em idiomas diferentes são unidades distintas) e **custo declarado**, com cadeia de fallback explícita (condição+tipo → condição → tipo → mercado geral) e rótulo dizendo de onde veio o número. A referência graduada casa a **mesma escala** (PSA 10 ≠ PSA 5) antes de casar só a certificadora, e entrou o cartão "Lucro sobre o que você pagou" | `7121992` |
...
| **Fix NavBar transparente (Tailwind v4)** — sintaxe de var do v3 (`bg-[--color-x]`) gerava CSS inválido (`background-color:--color-x`) → fundo transparente, conteúdo aparecia atrás da barra fixa ao rolar. Corrigidas 21 ocorrências (NavBar.tsx + page.tsx) para `[var(--color-x)]` | `66595a3` |
