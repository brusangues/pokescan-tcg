    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Histórico de commits e experimentos do modelo</p>
        </div>
      </div>
      ...
      {loading && commits === null ? (
        <div className="text-center py-16 text-gray-400">Carregando changelog...</div>
      ) : (
        <>
          {/* ── Ablações ── */}
          <section className="mb-10">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-gray-900">Ablações de embeddings</h2>
              <span className="text-xs text-gray-400">({ablations?.length ?? 0} configs · split temporal)</span>
            </div>
            ...
            {melhor && (
              <div className="flex items-center gap-3 bg-gradient-to-r from-amber-50 to-yellow-50 border border-amber-200 rounded-xl px-4 py-3 mb-4">
                ...
                <div className="text-sm">
                  <span className="font-semibold text-gray-900">Melhor config:</span>{' '}
                  ...
                  <span className="text-gray-500"> — R² {melhor.r2.toFixed(4)} · MAE ${melhor.mae.toFixed(2)}</span>
                </div>
              </div>
            )}
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {ablations?.map((a, i) => {
                      ...
                      return (
                        <tr key={a.label} className={`border-b border-gray-100 last:border-0 ${isBest ? 'bg-amber-50/60' : i % 2 ? 'bg-gray-50/40' : ''}`}>
                          <td className="px-4 py-2.5">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-gray-400">·</span>
                              <span className="text-gray-500">PCA{a.pca}</span>
                            </div>
                          </td>
                          <td className="px-4 py-2.5 text-right font-mono text-gray-700">${a.mae.toFixed(2)}</td>
                          <td className="px-4 py-2.5 text-right font-mono font-semibold text-gray-900">{a.r2.toFixed(4)}</td>
                          <td className="px-4 py-2.5 text-right text-gray-500">{a.n_train.toLocaleString('pt-BR')}</td>
                          <td className="px-4 py-2.5 text-right text-gray-500">{a.n_test.toLocaleString('pt-BR')}</td>
                        </tr>
                      );
                    })}
                  </tbody>
          ...
          <section>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-lg font-semibold text-gray-900">Histórico de commits</h2>
              <span className="text-xs text-gray-400">({commits?.length ?? 0} recentes · branch hermes)</span>
            </div>
              ...
              <ul className="divide-y divide-gray-100">
                {commits?.map(c => (
                  <li key={c.hash} className="px-4 py-3 flex items-start gap-3">
                    <span className="font-mono text-xs text-gray-400 mt-1 w-16 shrink-0">{c.hash}</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm text-gray-800 font-medium leading-snug">{c.subject}</p>
                      <p className="text-xs text-gray-400 mt-0.5">{fmtData(c.date)} · {c.author}</p>
                    </div>
